...
ROpticsOpt::ROpticsOpt(const char *name, const char *description,
	...
	//fPointingOffset, HCS
	fPointingOffset.SetXYZ(-a * MissPointZ * TMath::Cos(HRSAngle), MissPointY,
			-MissPointZ * TMath::Sin(HRSAngle));
	...
	DEBUG_INFO("ROpticsOpt",
			"HRS @ %f Degree, PointingOffset = (%f,%f,%f), SievePos = (%f,%f,%f)",
			HRSAngle / TMath::Pi() * 180, fPointingOffset.X(),
			fPointingOffset.Y(), fPointingOffset.Z(), SieveOffX, SieveOffY,
			ZPos);
...
// Database I/O
///////////////////////////////////////////////////////////////////////////////
Double_t ROpticsOpt::TravelLength(TVector3 ReactionVertex,
...
//assume we know the vetex point, beam position X, beam position Y and CalcReacZ
	...
	Double_t vx = ReactionVertex.X(), vy = ReactionVertex.Y(), vz =
			ReactionVertex.Z(); // vertex position
	...
	distance_tip = TMath::Sqrt(
			TMath::Power(vx - tx, 2) + TMath::Power(vy - ty, 2)
					+ TMath::Power(vz - tz, 2));
	...
	cos_angleR = (v_tipcenter.Dot(MomDirectionHCS))
			/ (v_tipcenter.Mag() * MomDirectionHCS.Mag());
...
Double_t ROpticsOpt::ElossTargetBefore(TVector3 ReactionVertex,
		TVector3 MomDirectionHCS) {
...
Double_t ROpticsOpt::ElossTargetAfter(TVector3 ReactionVertex,
		TVector3 MomDirectionHCS) {
	...
	if (z < (TL / 2. - R)) {
		D = TMath::Sqrt(
				(x - vx) * (x - vx) + (y - vy) * (y - vy)
						+ (z - vz) * (z - vz));
		ElossAfter = D * LossEachUnitA
				+ LH2_Thickness_Side / TMath::Sqrt(thp * thp + php * php)
						* TMath::Sqrt(thp * thp + php * php + 1)
						* LossEachUnitA_Al7075;
		...
		D = R / sin(angleR) * sin(angleD);
		ElossAfter = D * LossEachUnitA
				+ LH2_Thickness_Tip / cos(anglex) * LossEachUnitA_Al7075;
...
Int_t ROpticsOpt::LoadDataBase(TString DataBaseName) {
	...
	if (!file) {
		...
		DEBUG_INFO("LoadDataBase", "Parsing Database %s", DataBaseName.Data());
	...
	while (fgets(buff, LEN, file)) {
		...
		vsiz_t p_cnt;
		for (p_cnt = 0;
				pos < line_spl.size() && p_cnt < kPORDER
						&& pos <= npow + kPORDER; pos++, p_cnt++) {
		...
		if (p_cnt < 1) {
			Error(Here(here), "Could not read in Matrix Element %s%d%d%d!", w,
					ME.pw[0], ME.pw[1], ME.pw[2]);
		...
		if (mat) {
			...
			if (mat == &fFPMatrixElems) {
				if (ME.pw[0] == 0 && ME.pw[1] == 0 && ME.pw[2] == 0) {
					...
					if (m.order > 0) {
						Warning(Here(here),
								"Duplicate definition of focal plane matrix element: %s. Using first definition.",
								buff);
					} else
						m = ME;
				} else
					Warning(Here(here),
							"Bad coefficients of focal plane matrix element %s",
							buff);
			} else {
				...
				if (match) {
					Warning(Here(here),
							"Duplicate definition of matrix element: %s. Using first definition.",
							buff);
				} else
					mat->push_back(ME);
			}
...
// shujie: save a copy of output in newDB format for analyzer 1.6.
//         for now the code still take the old format for optimization
Int_t ROpticsOpt::SaveNewDataBase(const TString DataBaseName) {
	// Output database in memory to new database file
	// WARNING: Hard coded text included
	const TString NewDataBaseName = DataBaseName + ".new";
	DEBUG_INFO("SaveNewDataBase", "Saving to %s", NewDataBaseName.Data());
    std::cout<<"\n ***********************\n Save To ::\n"<<NewDataBaseName.Data()<<"\n**************"<<std::endl;
	typedef vector<THaMatrixElement>::size_type vsiz_t;

	ofstream *file = new ofstream;
	file->open(NewDataBaseName.Data());
	//FILE* file = fopen(DataBaseName, "w");
	if (!file->is_open()) {
		Error("SaveNewDataBase", "Cannot open file %s", NewDataBaseName.Data());
		return kFileError;
	}
	TDatime dt;
	//fprintf(file, "# -------------------------------------------------------------");
	*file << "# -------------------------------------------------------------"
			<< endl;
	*file << "# Optimized by Shujie Li @ " << dt.AsString() << endl;
	*file << "# Saved to " << NewDataBaseName << endl;
	*file << OldComments;

//    *file << NewDatabaseHeader;

	//*file << scientific << setprecision(6);
	*file << scientific << uppercase << setprecision(6);
	...
	for (vsiz_t i = 0; i < fDMatrixElems.size(); i++) {
		*file << "D ";
		...
		//int j;
		for (int j = 0; j < int(m.OptOrder); j++) {
			//fprintf(file, " %13.6e", m.poly[j]);
			//*file << " " << setw(13) << m.poly[j];
			*file << " " << setw(11) << m.poly[j];
		}
		//for (; j < kPORDER; j++) {
		//fprintf(file, " %13.6e", 0.0);
		//    *file << setw(13) << m.poly[j];
...
UInt_t ROpticsOpt::Matrix2Array(Double_t Array[],
		const std::vector<THaMatrixElement> &Matrix, Bool_t FreeParaFlag[]) {
	// Matrix -> Array
	...
	for (vsiz_t i = 0; i < Matrix.size(); i++) {
		...
		for (j = 0; (int) j < m.order; j++) {
			if (FreeParaFlag)
				FreeParaFlag[idx] = j < m.OptOrder ? kTRUE : kFALSE;
			Array[idx++] = m.poly[j];
		}
		for (; j < kPORDER; j++) {
			if (FreeParaFlag)
...
UInt_t ROpticsOpt::Array2Matrix(const Double_t Array[],
		std::vector<THaMatrixElement> &Matrix) {
	...
	assert(max_j >= 0);
	DEBUG_INFO("Array2Matrix", "Load Size = %d, %d", max_i, Matrix.size());
	THaMatrixElement &m = Matrix[max_i];
	DEBUG_INFO("Array2Matrix",
			"Load Size = %d, max ele = (%d %d %d %d) = %f, average = %f", idx,
			max_j, m.pw[0], m.pw[1], m.pw[2], m.poly[max_j], sumele / idx);
...
UInt_t ROpticsOpt::LoadRawData(TString DataFileName, UInt_t NLoad,
		UInt_t MaxDataPerGroup) {
	...
	while (fgets(buff, LEN, file)) {
		...
		if (NLineRead % 100000 == 0)
			DEBUG_INFO("LoadRawData", "%d/%d Entries Loaded", NRead, NLineRead);
		...
		if (grpcnt > MaxDataPerGroup) {
			DEBUG_MASSINFO("LoadRawData",
					"ignore data %d from cutid %d (%d ev total)", NLineRead,
					cutid, grpcnt);
			continue;
		}
...
#if DEBUG_LEVEL>=4
		std::cout << "DataID: " << cutid << "	kx: " << eventdata[kX] << "	kth:"
				<< eventdata[kTh] << "	ky:" << eventdata[kY] << "	kphi:"
				<< eventdata[kPhi] << std::endl;
#endif
	...
	for (int i = 0; i < kMaxDataGroup; i++) {
		...
		if (datagrpcnt[i] > MaxDataPerGroup)
			goodstatcut++;
	}

	DEBUG_INFO("LoadRawData",
			"Event Limit/Cut = %d, %d / %d ev read, %d / %d cut have enough ev",
...
const TVector3 ROpticsOpt::GetSieveHoleTCS(UInt_t Col, UInt_t Row) {
	...
	// temporary uasage
	// for prex experiment,
	const Double_t PositionIndex_y[] = { -20.52, -18.12, -15.72, -10.92, -8.52,
			-6.12, 0.0, 3.06, 6.12, 12.24, 15.30, 18.30, 21.42 };
...
const TVector3 ROpticsOpt::GetSieveHoleTCS_PRex(TString hrs, UInt_t Col,
		UInt_t Row) {
	// return the Sieve hole position
	assert(Col < NSieveCol);
	assert(Row < NSieveRow);
...
const TVector3 ROpticsOpt::GetSieveHoleCorrectionTCS(UInt_t nfoil, UInt_t Col,
		UInt_t Row) {
	assert(nfoil < NFoils);
	assert(Col < NSieveCol);
	assert(Row < NSieveRow);
	...
	const TVector3 BeamSpotHCS_average(BeamX_average, BeamY_average,
			targetfoils[nfoil]);
	const TVector3 BeamSpotTCS_average = fTCSInHCS.Inverse()
			* (BeamSpotHCS_average - fPointingOffset);
	...
	const TVector3 SieveHoleCorrectionTCS((Xreal_p + Xreal_m) / 2,
			(Yreal_p + Yreal_m) / 2, SieveHoleTCS.Z());
...
//    TH2F * HSieveCalcThetaPhi = new TH2F("Calck Theta vs Phi","Calck Theta vs Phi", 500,-0.1,0.1, 500,-0.1, 0.1);
	...
	DEBUG_INFO("PrepareSieve",
			"Calculating real target theta and phi for each event based on survey information.");
...
//		const TVector3 SieveHoleTCS =GetSieveHoleCorrectionTCS(FoilID, Col, Row); ;//GetSieveHoleTCS_PRex("R",Col,Row);
		...
		const TVector3 BeamSpotHCS(eventdata.Data[kBeamX],
				eventdata.Data[kBeamY], targetfoils[FoilID]);
		//        const TVector3 BeamSpotHCS(BeamX_average, BeamY_average, targetfoils[FoilID]);
		eventdata.Data[kBeamZ] = targetfoils[FoilID];
		const TVector3 BeamSpotTCS = fTCSInHCS.Inverse()
				* (BeamSpotHCS - fPointingOffset);
		...
		const Double_t x_tg = BeamSpotTCS.X()
				- BeamSpotTCS.Z() * eventdata.Data[kRealTh];
		const Double_t y_tg = BeamSpotTCS.Y()
				- BeamSpotTCS.Z() * eventdata.Data[kRealPhi];
		...
		eventdata.Data[kRealThMatrix] = eventdata.Data[kRealTh]
				- x_tg * ExtTarCor_ThetaCorr;

		exttargcorr_th += x_tg * ExtTarCor_ThetaCorr;
		rms_exttargcorr_th += x_tg * ExtTarCor_ThetaCorr * x_tg
				* ExtTarCor_ThetaCorr;

		DEBUG_MASSINFO("PrepareSieve", "Real_Th_Matrix = %f,\t Real_Phi = %f",
				eventdata.Data[kRealThMatrix], eventdata.Data[kRealPhi]);

		std::cout << (UInt_t) eventdata.Data[kCutID] << "  Col Row:(" << Col
				<< "," << Row << ")  Pos=> (" << SieveHoleTCS.X() << ", "
				<< SieveHoleTCS.Y() << ", " << SieveHoleTCS.Z() << ")   ";
		std::cout << "kRealTh:" << MomDirectionTCS.X() / MomDirectionTCS.Z()
				<< "  kRealPhi:" << MomDirectionTCS.Y() / MomDirectionTCS.Z()
				<< std::endl;

	}

	DEBUG_INFO("PrepareSieve",
			"Average Extended Target Correction: th = %f,\t rms_th = %f",
			exttargcorr_th / fNRawData,
			TMath::Sqrt(rms_exttargcorr_th / fNRawData));
...
TCanvas* ROpticsOpt::CheckSieve(Int_t PlotFoilID) {
	...
	for (uint i = 0 ; i < NKine; i ++){
		HSieveMomRealThetaPhi[i]=new TH2F(Form("dpID_%d\%_Target_th.vs.ph",-2+i),Form("dpID_%d\%_Target_th.vs.ph",-2+i),500, -0.03, 0.03, 500, -0.04, 0.04);
	...
	for (UInt_t idx = 0; idx < NFoils; idx++) {
		...
		HSieveRealThetaPhi[idx] = new TH2F(
				...
				Form("Sieve Plane Real. (tg_Theta vs tg_Phi) for Data set #%d",
						idx),500, -0.03, 0.03, 500, -0.04, 0.04);
...
TCanvas* ROpticsOpt::CheckSieveAccu(Int_t PlotFoilID) {
	...
	for (UInt_t idx = 0; idx < NFoils; idx++) {
		for (UInt_t idy = 0; idy < NSieveCol; idy++) {
			HSievePX[idx][idy] = new TH1D(Form("X_Foil%d_Col%d", idx, idy),
					Form("X_Foil%d_Col%d", idx, idy), 400, -y_lim[0], y_lim[0]);
	...
	for (UInt_t idx = 0; idx < NFoils; idx++) {
		for (UInt_t idy = 0; idy < NSieveRow; idy++) {
			HSievePY[idx][idy] = new TH1D(Form("Y_Foil%d_Row%d", idx, idy),
					Form("Y_Foil%d_Row%d", idx, idy), 400, -x_lim[0], x_lim[0]);
	...
	Double_t SieveInfo[NFoils][NSieveCol][NSieveRow][15] = { { { { 0 } } } };
	...
	for (UInt_t idx = 0; idx < fNRawData; idx++) {
		...
		SieveEventID[FoilID][Col][Row][kRealSieveX] =
				SieveHoleCorrectionTCS.X();
		SieveEventID[FoilID][Col][Row][kRealSieveY] =
				SieveHoleCorrectionTCS.Y();
		SieveEventID[FoilID][Col][Row][kCalcSieveX] = ProjectionX;
		SieveEventID[FoilID][Col][Row][kCalcSieveY] = ProjectionY;
	...
	DEBUG_INFO("CheckSieveAccu", "Average : D_X = %f,\t D_Y = %f",
			dX / fNRawData, dY / fNRawData);
	...
	for (UInt_t nfoil = 0; nfoil < NFoils; nfoil++) {
		for (UInt_t idx = 0; idx < NSieveCol; idx++) {
			for (UInt_t idy = 0; idy < NSieveRow; idy++) {
				...
				gXmin[idx][idy] = SieveEventID[nfoil][idx][idy][kRealSieveX]
						- xrange * SieveRadius;
				gXmax[idx][idy] = SieveEventID[nfoil][idx][idy][kRealSieveX]
						+ xrange * SieveRadius;
				gYmin[idx][idy] = SieveEventID[nfoil][idx][idy][kRealSieveY]
						- yrange * SieveRadius + yoffset;
				gYmax[idx][idy] = SieveEventID[nfoil][idx][idy][kRealSieveY]
						+ yrange * SieveRadius + yoffset;
				gX[nfoil][idx][idy] = new TF1(
						Form("gX_F%d_C%d_R%d", nfoil, idx, idy), "gaus",
						gXmin[idx][idy], gXmax[idx][idy]);
				gY[nfoil][idx][idy] = new TF1(
						Form("gY_F%d_C%d_R%d", nfoil, idx, idy), "gaus",
						gYmin[idx][idy], gYmax[idx][idy]);
		...
		for (UInt_t idx = 0; idx < NSieveCol; idx++) {
			for (UInt_t idy = 0; idy < NSieveRow; idy++) {
				if (SieveEventID[nfoil][idx][idy][kEventID] > 0) {
					...
					HSievePY[nfoil][idy]->Fit(
							Form("gY_F%d_C%d_R%d", nfoil, idx, idy), "R+");
					SieveInfo[nfoil][idx][idy][kYmean] =
							HSievePY[nfoil][idy]->GetFunction(
									Form("gY_F%d_C%d_R%d", nfoil, idx, idy))->GetParameter(
									1);
					SieveInfo[nfoil][idx][idy][kYmeanErr] =
							HSievePY[nfoil][idy]->GetFunction(
									Form("gY_F%d_C%d_R%d", nfoil, idx, idy))->GetParError(
									1);
					SieveInfo[nfoil][idx][idy][kYamp] =
							HSievePY[nfoil][idy]->GetFunction(
									Form("gY_F%d_C%d_R%d", nfoil, idx, idy))->GetParameter(
									2);
					SieveInfo[nfoil][idx][idy][kYampErr] =
							HSievePY[nfoil][idy]->GetFunction(
									Form("gY_F%d_C%d_R%d", nfoil, idx, idy))->GetParError(
									2);
					SieveInfo[nfoil][idx][idy][kYndf] =
							HSievePY[nfoil][idy]->GetFunction(
									Form("gY_F%d_C%d_R%d", nfoil, idx, idy))->GetNDF();
					SieveInfo[nfoil][idx][idy][kYchi2] =
							HSievePY[nfoil][idy]->GetFunction(
									Form("gY_F%d_C%d_R%d", nfoil, idx, idy))->GetChisquare();
					//	   SieveInfo[nfoil][idx][idy][13] = SieveInfo[nfoil][idx][idy][1]-(SieveEventID[nfoil][idx][idy][kRealSieveY]);
					SieveInfo[nfoil][idx][idy][kYdiff] =
							SieveInfo[nfoil][idx][idy][1]
									- (GetSieveHoleTCS(idx, idy).Y());
			...
			for (UInt_t idy = 0; idy < NSieveCol; idy++) {
				if (SieveEventID[nfoil][idy][idx][kEventID] > 0) {
					TLine *line = new TLine(
							SieveEventID[nfoil][idy][idx][kRealSieveY], 0,
							SieveEventID[nfoil][idy][idx][kRealSieveY], ymax);
		...
		for (UInt_t idx = 0; idx < NSieveCol; idx++) {
			for (UInt_t idy = 0; idy < NSieveRow; idy++) {
				if (SieveEventID[nfoil][idx][idy][kEventID] > 0) {
					...
					HSievePX[nfoil][idx]->Fit(
							Form("gX_F%d_C%d_R%d", nfoil, idx, idy), "R+");
					SieveInfo[nfoil][idx][idy][kXmean] =
							HSievePX[nfoil][idx]->GetFunction(
									Form("gX_F%d_C%d_R%d", nfoil, idx, idy))->GetParameter(
									1);
					SieveInfo[nfoil][idx][idy][kXmeanErr] =
							HSievePX[nfoil][idx]->GetFunction(
									Form("gX_F%d_C%d_R%d", nfoil, idx, idy))->GetParError(
									1);
					SieveInfo[nfoil][idx][idy][kXamp] =
							HSievePX[nfoil][idx]->GetFunction(
									Form("gX_F%d_C%d_R%d", nfoil, idx, idy))->GetParameter(
									2);
					SieveInfo[nfoil][idx][idy][kXampErr] =
							HSievePX[nfoil][idx]->GetFunction(
									Form("gX_F%d_C%d_R%d", nfoil, idx, idy))->GetParError(
									2);
					SieveInfo[nfoil][idx][idy][kXndf] =
							HSievePX[nfoil][idx]->GetFunction(
									Form("gX_F%d_C%d_R%d", nfoil, idx, idy))->GetNDF();
					SieveInfo[nfoil][idx][idy][kXchi2] =
							HSievePX[nfoil][idx]->GetFunction(
									Form("gX_F%d_C%d_R%d", nfoil, idx, idy))->GetChisquare();
					//	    SieveInfo[nfoil][idx][idy][14] = SieveInfo[nfoil][idx][idy][5]-(SieveEventID[nfoil][idx][idy][kRealSieveX]);
					SieveInfo[nfoil][idx][idy][kXdiff] =
							SieveInfo[nfoil][idx][idy][5]
									- (GetSieveHoleTCS(idx, idy).X());
				}
			}
		}
	}
	...
	for (UInt_t nfoil = 0; nfoil < NFoils; nfoil++) {
		for (UInt_t idx = 0; idx < NSieveCol; idx++) {
			...
			for (UInt_t idy = 0; idy < NSieveRow; idy++) {
				if (SieveEventID[nfoil][idx][idy][kEventID] > 0) {
					TLine *line = new TLine(
							SieveEventID[nfoil][idx][idy][kRealSieveX], 0,
							SieveEventID[nfoil][idx][idy][kRealSieveX], ymax);
					line->SetLineColor(kBlue);
					line->Draw();
				}
			}
	...
	for (UInt_t nfoil = 0; nfoil < NFoils; nfoil++) {
		...
		c3 = new TCanvas(Form("SieveCheck Y #foil %d", nfoil),
				Form("SieveCheck Y #foil %d", nfoil), 1800, 1100);
		...
		for (UInt_t idx = 0; idx < NSieveRow; idx++) {
			...
			for (UInt_t idy = 0; idy < NSieveCol; idy++) {
				if (SieveEventID[nfoil][idy][idx][kEventID] > 0) {
					...
					line->Draw("same");
					tt = new TPaveText(
							SieveEventID[nfoil][idy][idx][kRealSieveY] - 0.003,
							0.95 * ymax,
							SieveEventID[nfoil][idy][idx][kRealSieveY] + 0.0025,
							ymax * 1.13);
					//	       tt->SetLineColor(0);
					tt->SetFillColor(0);
					tt->SetShadowColor(0);
					tt->AddText(
							Form("%1.2f",
									1000
											* (SieveEventID[nfoil][idy][idx][kRealSieveY]
													- SieveInfo[nfoil][idy][idx][kYmean])));
					tt->Draw();
				}
			}
		}
	...
	for (UInt_t nfoil = 0; nfoil < NFoils; nfoil++) {
		...
		c3 = new TCanvas(Form("SieveCheck X #foil %d", nfoil),
				Form("SieveCheck X #foil %d", nfoil), 1800, 1100);
		...
		for (UInt_t idy = y1; idy < y2; idy++) {
			...
			for (UInt_t idx = 0; idx < NSieveRow; idx++) {
				if (SieveEventID[nfoil][idy][idx][kEventID] > 0) {
					TLine *line = new TLine(
							SieveEventID[nfoil][idy][idx][kRealSieveX], 0,
							SieveEventID[nfoil][idy][idx][kRealSieveX], ymax);
					line->SetLineColor(kBlue);
					line->Draw("same");
					tt = new TPaveText(
							SieveEventID[nfoil][idy][idx][kRealSieveX] - 0.007,
							0.95 * ymax,
							SieveEventID[nfoil][idy][idx][kRealSieveX] + 0.007,
							ymax * 1.13);
					//	       tt->SetLineColor(0);
					...
					tt->AddText(
							Form("%1.2f",
									1000
											* (SieveEventID[nfoil][idy][idx][kRealSieveX]
													- SieveInfo[nfoil][idy][idx][kXmean])));
					tt->Draw();
				}
			}
		}
	...
	for (UInt_t nfoil = 0; nfoil < NFoils; nfoil++) {
		fprintf(fp2,
				Form(" The horizontal correction of each hole in Foil#%d.\n",
						nfoil + 1));
		for (UInt_t idx = 0; idx < NSieveRow; idx++) {
			for (UInt_t idy = 0; idy < NSieveCol; idy++) {
				fprintf(fp2, "%*.2f", 6,
						1000
								* (GetSieveHoleCorrectionTCS(nfoil,
										NSieveCol - 1 - idy,
										NSieveRow - 1 - idx).Y()
										- GetSieveHoleTCS(NSieveCol - 1 - idy,
												NSieveRow - 1 - idx).Y()));
			}
			fprintf(fp2, "\n");
		}
	...
	for (UInt_t nfoil = 0; nfoil < NFoils; nfoil++) {
		fprintf(fp2,
				Form(" The vertical correction of each hole in Foil#%d.\n",
						nfoil + 1));
		for (UInt_t idx = 0; idx < NSieveRow; idx++) {
			for (UInt_t idy = 0; idy < NSieveCol; idy++) {
				fprintf(fp2, "%*.2f  ", 6,
	...
	for (UInt_t nfoil = 0; nfoil < NFoils; nfoil++) {
		fprintf(fp1,
				Form("      The number of events in each hole in Foil#%d.\n",
						nfoil + 1));
		for (UInt_t idx = 0; idx < NSieveRow; idx++) {
			for (UInt_t idy = 0; idy < NSieveCol; idy++) {
				fprintf(fp1, "%5.0f  ",
						SieveInfo[nfoil][NSieveCol - 1 - idy][NSieveRow - 1
								- idx][0]);
				if (SieveInfo[nfoil][NSieveCol - 1 - idy][NSieveRow - 1 - idx][0]
						> 0) {
					NHole[nfoil]++;
					NHOLE++;
				}
	...
	for (UInt_t nfoil = 0; nfoil < NFoils; nfoil++) {
		fprintf(fp1,
				Form(
						" The difference of horizontal fitting mean and real position for each hole in Foil#%d.\n",
						nfoil + 1));
		//SieveYbyCol[]
		...
		for (UInt_t idx = 0; idx < NSieveRow; idx++) {
			for (UInt_t idy = 0; idy < NSieveCol; idy++) {
				a1 =
						GetSieveHoleTCS(NSieveCol - 1 - idy,
								NSieveRow - 1 - idx).Y();
				a2 = GetSieveHoleCorrectionTCS(nfoil, NSieveCol - 1 - idy,
						NSieveRow - 1 - idx).Y();
				if (SieveInfo[nfoil][NSieveCol - 1 - idy][NSieveRow - 1 - idx][0]
						> 0) {
					fprintf(fp1, "%-6.2f/%-6.2f  ",
							1000
									* (SieveInfo[nfoil][NSieveCol - 1 - idy][NSieveRow
											- 1 - idx][13]), 1000 * (a2 - a1));
					rms_hole_h[nfoil] += TMath::Power(
							SieveInfo[nfoil][NSieveCol - 1 - idy][NSieveRow - 1
									- idx][13] - a2 + a1, 2);
					RMS_hole_h += TMath::Power(
							SieveInfo[nfoil][NSieveCol - 1 - idy][NSieveRow - 1
									- idx][13] - a2 + a1, 2);
				} else {
	...
	for (UInt_t nfoil = 0; nfoil < NFoils; nfoil++) {
		fprintf(fp1,
				Form(
						" The difference of vertical fitting mean and real position for each hole in Foil#%d.\n",
						nfoil + 1));
		for (UInt_t idx = 0; idx < NSieveRow; idx++) {
				...
				if (SieveInfo[nfoil][NSieveCol - 1 - idy][NSieveRow - 1 - idx][0]
					...
					fprintf(fp1, "%-6.2f/%-6.2f  ",
							1000
									* SieveInfo[nfoil][NSieveCol - 1 - idy][NSieveRow
											- 1 - idx][14], 1000 * (a2 - a1));
					rms_hole_v[nfoil] += TMath::Power(
							SieveInfo[nfoil][NSieveCol - 1 - idy][NSieveRow - 1
									- idx][14] - a2 + a1, 2);
					RMS_hole_v += TMath::Power(
							SieveInfo[nfoil][NSieveCol - 1 - idy][NSieveRow - 1
									- idx][14] - a2 + a1, 2);
				} else {
					fprintf(fp1, "x.xx  /x.xx    ");
...
Double_t ROpticsOpt::SumSquareDTh() {
	...
	// why they add some new informations, seems like unnecessary
	...
	for (UInt_t idx = 0; idx < fNRawData; idx++) {
		...
		rmsth += (theta - eventdata.Data[kRealThMatrix])
				* (theta - eventdata.Data[kRealThMatrix]);

		DEBUG_MASSINFO("SumSquareDTh", "D_Th = %f = \t%f - \t%f",
				theta - eventdata.Data[kRealThMatrix], theta,
...
Double_t ROpticsOpt::SumSquareDPhi() {
	...
	for (UInt_t idx = 0; idx < fNRawData; idx++) {
		...
		rmsphi += (phi - eventdata.Data[kRealPhi])
				* (phi - eventdata.Data[kRealPhi]);

		DEBUG_MASSINFO("SumSquareDPhi", "D_Phi = %f = \t%f - \t%f",
				phi - eventdata.Data[kRealPhi], phi, eventdata.Data[kRealPhi]);
...
//		const UInt_t Row = res % (NSieveRow); //starting 0!
		...
		TVector3 BeamSpotHCS(eventdata.Data[kBeamX], eventdata.Data[kBeamY],
				targetfoils[FoilID]);
		//	TVector3 BeamSpotHCS(BeamX_average, BeamY_average , targetfoils[FoilID]);
		eventdata.Data[kBeamZ] = targetfoils[FoilID];
		TVector3 BeamSpotTCS = fTCSInHCS.Inverse()
				* (BeamSpotHCS - fPointingOffset);
...
TCanvas* ROpticsOpt::CheckVertex() {
	...
	for (UInt_t idx = 0; idx < NFoils; idx++) {
		HTgY[idx] = new TH1D(Form("Target_Y%d", idx),
				Form("Target Y for Foil Target #%d", idx), 500, -YRange,
				YRange);
		HTgYReal[idx] = new TH1D(Form("Target_Y%d", idx),
				Form("Target Y for Foil Target #%d", idx), 500, -YRange,
				YRange);
		...
		for (UInt_t ncol = 0; ncol < NSieveCol; ncol++) {
			for (UInt_t nrow = 0; nrow < NSieveRow; nrow++) {
				HTgYHole[idx][ncol][nrow] = new TH1D(
						Form("TgY%d_%d_%d", idx, ncol, nrow),
						Form("TgY%d_%d_%d", idx, ncol, nrow), 400, -YRange,
						YRange);
				HTgYHoleReal[idx][ncol][nrow] = new TH1D(
						Form("TgY%d_%d_%d", idx, ncol, nrow),
						Form("TgY%d_%d_%d", idx, ncol, nrow), 400, -YRange,
						YRange);
	...
	for (UInt_t idx = 0; idx < fNRawData; idx++) {
		...
		dtg_y_rms += (eventdata.Data[kCalcTgY] - eventdata.Data[kRealTgY])
				* (eventdata.Data[kCalcTgY] - eventdata.Data[kRealTgY]);
	}

	DEBUG_INFO("CheckTgY", "dtg_v = %f,\t dtg_v_rms = %f", dtg_y / fNRawData,
			TMath::Sqrt(dtg_y_rms / fNRawData));
	...
	for (UInt_t idx = 0; idx < nplot; idx++) {
		...
		HTgY[idx]->GetXaxis()->SetRangeUser(YMean[idx] - 0.015,
				YMean[idx] + 0.015);
		HTgYReal[idx]->GetXaxis()->SetRangeUser(YMean[idx] - 0.015,
				YMean[idx] + 0.015);
		...
		TF1 *f = new TF1(FitFunc, "gaus",
				ymean - DefResolution * FitRangeMultiply,
				ymean + DefResolution * FitRangeMultiply);
	...
	for (UInt_t idx = 0; idx < NFoils; idx++) {
		HTgZ[idx] = new TH1D(Form("Target_Z%d", idx),
				Form("Target Z for Foil Target #%d", idx), 500, -ZRange,
				ZRange);
		HTgZReal[idx] = new TH1D(Form("Target_Z%d", idx),
				Form("Target Z for Foil Target #%d", idx), 500, -ZRange,
				ZRange);
	...
	for (UInt_t idx = 0; idx < fNRawData; idx++) {
		...
		const Int_t a = (HRSAngle > 0) ? 1 : -1;
		CalcReacZ = -(eventdata.Data[kCalcTgY] - a * MissPointZ)
				* TMath::Cos(TMath::ATan(Real_Tg_Phi))
				/ TMath::Sin(HRSAngle + TMath::ATan(Real_Tg_Phi))
				+ BeamSpotHCS.X()
						* TMath::Cos(HRSAngle + TMath::ATan(Real_Tg_Phi))
						/ TMath::Sin(HRSAngle + TMath::ATan(Real_Tg_Phi));
		...
		dtg_z_rms += (CalcReacZ - eventdata.Data[kRealReactZ])
				* (CalcReacZ - eventdata.Data[kRealReactZ]);
	}

	DEBUG_INFO("CheckTgZ", "dtg_z = %f,\t dtg_z_rms = %f", dtg_z / fNRawData,
			TMath::Sqrt(dtg_z_rms / fNRawData));
	...
	for (UInt_t idx = 0; idx < nplot; idx++) {
		...
		//	cout<<"mean:"<<mean<<endl;
		//	zmean = HTgZ[idx]->GetMean();
		HTgZ[idx]->GetXaxis()->SetRangeUser(ZMean[idx] - 0.02,
				ZMean[idx] + 0.02);
		HTgZReal[idx]->GetXaxis()->SetRangeUser(ZMean[idx] - 0.02,
				ZMean[idx] + 0.02);

		TString FitFunc1 = Form("ZtPeak%d", idx);
		TF1 *f1 = new TF1(FitFunc1, "gaus",
				ZMean[idx] - DefResolution * FitRangeMultiply,
				ZMean[idx] + DefResolution * FitRangeMultiply);
		...
		tt->AddText(
				Form("\\Delta = %2.1f mm",
						1000 * (f1->GetParameter(1) - targetfoils[idx])));
...
Double_t ROpticsOpt::SumSquareDTgY(void) {
	...
	for (UInt_t idx = 0; idx < fNRawData; idx++) {
		...
		const Double_t ArbitaryVertexShift = fArbitaryVertexShift[FoilID]
				* (-TMath::Sin(HRSAngle));
...
void ROpticsOpt::PrepareDp(void) {
	...
	for (UInt_t idx = 0; idx < fNRawData; idx++) {
		...
		UInt_t DpID = dataID / (NSieveRow * (NSieveCol + 1));      //starting 0!  check check check check check check

		UInt_t KineID = DpID;

		fNCalibData++;
		eventdata.Data[kKineID] = KineID; // kinematics ID??
		eventdata.Data[kCentralp] = HRSCentralMom[KineID]; //centrol momentum
		...
		tgy = CalcTargetVar(fYMatrixElems, powers)
				+ CalcTargetVar(fYTAMatrixElems, powers);
...
//		phi = eventdata.Data[kRealPhi];
		...
		DEBUG_MASSINFO("PrepareDp", "RealTh = %f,\t RealPh = %f",
				eventdata.Data[kRealThMatrix], eventdata.Data[kRealPhi]);
		...
		CalcReacZ = -(tgy + MissPointZ) * TMath::Cos(TMath::ATan(Tg_Phi))
				/ TMath::Sin(HRSAngle + TMath::ATan(Tg_Phi))
				+ BeamSpotHCS.X() * TMath::Cos(HRSAngle + TMath::ATan(Tg_Phi))
						/ TMath::Sin(HRSAngle + TMath::ATan(Tg_Phi));
		if (CalcReacZ > 0.15 / 2.)
			CalcReacZ = 0.15 / 2.;
		if (CalcReacZ < -0.15 / 2.)
		...
		//************************************//
		TVector3 ReactionVertex(eventdata.Data[kBeamX], eventdata.Data[kBeamY],
				CalcReacZ);
		Double_t D1 = 0;
		Double_t D2 = 0;
...
//		eventdata.Data[kElossDp] = 0.0;
		...
		const Double_t DpKinOffsets = (ScatMom(DM, Ma,
				Ebeam[KineID] - ElossBeforeTg, ScatteringAngle)
				- ScatMom(DM, Ma, Ebeam[KineID] - ElossBeforeTg,
						TMath::Abs(HRSAngle))) / eventdata.Data[kCentralp];
		...
		eventdata.Data[kRealDpKin] = ScatMom(DM, Ma,
				Ebeam[KineID] - ElossBeforeTg, TMath::Abs(HRSAngle))
				/ eventdata.Data[kCentralp] - 1;
		//******************//
		eventdata.Data[kDpKinOffsets] = DpKinOffsets - eventdata.Data[kElossDp];
		//********************//
		DEBUG_MASSINFO("PrepareDp",
				"ScatterMom = %f,\t Centralp = %f,\t radloss = %f,\t ebeam = %f",
				ScatMom(DM, Ma, Ebeam[KineID], TMath::Abs(HRSAngle)),
				eventdata.Data[kCentralp], eventdata.Data[kElossDp],
				Ebeam[KineID]);

		eventdata.Data[kRealDpKinMatrix] = eventdata.Data[kRealDpKin]
				- x_tg / ExtTarCor_DeltaCorr;

		exttargcorr_dp += x_tg / ExtTarCor_DeltaCorr;
		rms_exttargcorr_dp += (x_tg / ExtTarCor_DeltaCorr)
				* (x_tg / ExtTarCor_DeltaCorr);
		...
		for (UInt_t ExcitID = 0; ExcitID < NExcitationStates; ExcitID++) {
			...
			eventdata.Data[kRealDpKinExcitations + ExcitID] = ScatMom(
					ExcitationEnergyList[ExcitID], Ma,
					Ebeam[KineID] - ElossBeforeTg, TMath::Abs(HRSAngle))
					/ eventdata.Data[kCentralp] - 1;
		}
	...
	DEBUG_INFO("PrepareDp", "%d out of %d data is for calibration", fNCalibData,
			fNRawData);
	DEBUG_INFO("PrepareDp", "Average : ScatteringAngle = %f",
			scatang / fNRawData / TMath::Pi() * 180);
	DEBUG_INFO("PrepareDp", "Average : Travel Length before scattering = %f",
			D1_sum / fNRawData);
	DEBUG_INFO("PrepareDp", "Average : Travel Length after scattering = %f",
			D2_sum / fNRawData);
	DEBUG_INFO("PrepareDp", "Average : ElossTg = %f", ElossTg_sum / fNRawData);
	DEBUG_INFO("PrepareDp", "Average : ElossAfter = %f",
			ElossAfter_sum / fNRawData);
	DEBUG_INFO("PrepareDp", "Average DpKinOffsets = %f, RMS DpKinOffsets = %f",
			dpkinoff / fNRawData, TMath::Sqrt(dpkinoff_rms / fNRawData));
	DEBUG_INFO("PrepareDp",
			"Average Extended Target Corretion: dp = %f,\t rms_dp = %f",
			exttargcorr_dp / fNRawData,
			TMath::Sqrt(rms_exttargcorr_dp / fNRawData));
...
TCanvas* ROpticsOpt::CheckDp() {
	...
	TCanvas *c2 = new TCanvas("Check reconstructed Z", "Check reconstructed Z",
			1800, 900);
	c2->Divide(3, 2);
	...
	TCanvas *c3 = new TCanvas("Check ElossTg", "Check ElossTg Before ", 1800,
			900);
	c3->Divide(3, 2);
	...
	TCanvas *c4 = new TCanvas("c4", "Check ElossTg vs ReactZ Before", 1800,
			900);
	/* c4->Divide(3, 2);
	...
	TCanvas *c6 = new TCanvas("c6", "Check ElossTg vs ReactZ Before", 1800,
			900);
	c6->Divide(3, 2);
...
//	TH1F *hDpKinCalibSieve[NKine][13][7];
	...
	std::map<uint8_t, std::map<uint8_t, std::map<uint8_t, TH1F *>>>hCalcMomRealSieve;  // the theoretical value, calculated from the scattered angle
...
//	TCanvas *SieveHoleDp= new TCanvas("CheckDp", "Check Dp Kin Reconstruction", 1800,
//			900);
//	SieveHoleDp->Divide(2,2);
//
//	for (UInt_t KineID = 0; KineID < NKine; KineID++) {
//		SieveHoleDp->cd(KineID + 1);
//		if (hDpKinCalibSieve.find(KineID) != hDpKinCalibSieve.end()) {
//			for (std::map<uint8_t, std::map<uint8_t, TH1F*>>::iterator iter =
//					hDpKinCalibSieve[KineID].begin();
//					iter != hDpKinCalibSieve[KineID].end(); iter++) {
...
//				TLatex *t1=new TLatex((ittter->second)->GetMean(1)-0.001,(ittter->second)->GetMean(2)+0.0015, Form("%2.4fGeV",hhRealSieveScatteredAngle[KineID][itter->first][ittter->first]->GetMean()));
	...
	TCanvas *sieveThetaphiCanvas_cal= new TCanvas("sieveThetaphiCanvasCal", "sieveThetaphiCanvasCal", 1800,
						900);
...
TCanvas* ROpticsOpt::CheckDpGlobal() {
	...
	for (UInt_t KineID = 0; KineID < NKine; KineID++) {
		hDpKinCalib[KineID] = new TH1D(Form("hDpKinCalib%d", KineID),
				"Dp_Kin for Delta Scan", NDpRange, -DpRange, DpRange);
		hDpKinAll[KineID] = new TH1D(Form("hDpKinAll%d", KineID),
				Form("Dp_Kin for Delta Scan Kine. #%d", KineID), NDpRange,
				-DpRange, DpRange);
	...
	for (UInt_t idx = 0; idx < fNRawData; idx++) {
		...
		if (!ExtraDataFlag) {
			hDpKinCalib[KineID]->Fill(
					eventdata.Data[kCalcDpKin] + eventdata.Data[kElossDp]);
			AverCalcDpKin[KineID] += eventdata.Data[kCalcDpKin]
					+ eventdata.Data[kElossDp];
			NEvntDpKin[KineID]++;
		}
		hDpKinAll[KineID]->Fill(
				eventdata.Data[kCalcDpKin] + eventdata.Data[kElossDp]);
	...
	for (UInt_t KineID = 0; KineID < NKine; KineID++) {
		...
		if (KineID == 0) {
			...
			hDpKinAll[KineID]->SetAxisRange(RealDpKin[KineID] - dpRange,
					RealDpKin[KineID] + dpRange);
...
Double_t ROpticsOpt::SumSquareDp(Bool_t IncludeExtraData) {
	...
	if (IncludeExtraData) {
		Warning("SumSquareDp",
				"Data Beyond selected excitation state is included in this calculation");
	}
	for (UInt_t idx = 0; idx < fNRawData; idx++) {
		Double_t dp, dp_kin;
		...
		if (eventdata.Data[kExtraDataFlag] > 0 && !IncludeExtraData)
			continue;
		NCalibData++;
		...
		//const UInt_t KineID = (UInt_t) (eventdata.Data[kKineID]);
		assert(KineID == (UInt_t) (eventdata.Data[kKineID]));
...
//		rms_dp += (dp_kin - eventdata.Data[kRealDpKinMatrix]
//				+ ArbitaryDpKinShift)
//				* (dp_kin - eventdata.Data[kRealDpKinMatrix]
//						+ ArbitaryDpKinShift);

		auto realAngleCalDp=eventdata.Data[kDpKinOffsets]+eventdata.Data[kRealDpKin];
		auto realAngleCalMom=realAngleCalDp*eventdata.Data[kCentralp]+eventdata.Data[kCentralp];
		rms_dp += ((dp * eventdata.Data[kCentralp]
					+ eventdata.Data[kCentralp]) - realAngleCalMom)
					* ((dp * eventdata.Data[kCentralp]
							+ eventdata.Data[kCentralp]) - realAngleCalMom);

		}
		DEBUG_MASSINFO("SumSquareDp", "d_dp = %f = \t%f - \t%f",
				dp_kin - eventdata.Data[kRealDpKinMatrix], dp_kin,
				eventdata.Data[kRealDpKinMatrix]);

		// save the results
		eventdata.Data[kCalcDpKinMatrix] = dp_kin;
		eventdata.Data[kCalcDpKin] = dp_kin
				+ eventdata.Data[kRealTgX] / ExtTarCor_DeltaCorr;
	}
	...
	printf("SumSquareDp: #%d : d_dp = %f, rms_dp = %f\n", NCall,
			d_dp / NCalibData, 100 * TMath::Sqrt(rms_dp / NCalibData));
...
void ROpticsOpt::CalcMatrix(const Double_t x,
		vector<THaMatrixElement> &matrix) {
...
Double_t ROpticsOpt::CalcTargetVar(const vector<THaMatrixElement> &matrix,
